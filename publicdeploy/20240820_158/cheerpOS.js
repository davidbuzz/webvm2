...
function rootMakeFileData(mp, path, mode, cb)
{
	// Only the root itself can be opened
	if(path != "/"){
		return cb(null);
	}
	var ret = new CheerpJFileData(mp, path, 0, 0, CheerpJFileData.S_IFDIR | 0777, /*lastModified*/0);
...
function devMakeFileData(mp, path, mode, cb)
{
	...
	{
		fileData = new CheerpJFileData(mp, path, 0xffffffff, 2, CheerpJFileData.S_IFCHR | 0666, /*lastModified*/0);
		fileData.mount = DevRandomInodeOps;
	}
	else if(path == "/null")
	{
		fileData = new CheerpJFileData(mp, path, 0xffffffff, 3, CheerpJFileData.S_IFCHR | 0666, /*lastModified*/0);
